...
class TeamController extends Controller
{
    ...
    {
        //First Level
        $first_level_users = User::where('ref_by', auth()->user()->id)->get();
        ...
        $totalPurchase = Purchase::whereIn('user_id', array_merge($first_ids->toArray(), $second_ids->toArray(), $third_ids->toArray()))->where('status', 'active')->sum('amount');
        ...
        $totalpurchase1 = Purchase::whereIn('user_id', $first_ids->toArray())->whereIn('status', ['active', 'inactive'])->sum('amount');
        $totalpurchase2 = Purchase::whereIn('user_id', $second_ids->toArray())->whereIn('status', ['active', 'inactive'])->sum('amount');
        $totalpurchase3 = Purchase::whereIn('user_id', $third_ids->toArray())->whereIn('status', ['active', 'inactive'])->sum('amount');
