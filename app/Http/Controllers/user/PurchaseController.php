...
class PurchaseController extends Controller
{
    ...
    public function purchaseConfirmation($id)
    {
        ...
        $userId = Auth::id();
        if (RateLimiter::tooManyAttempts("purchase:$userId", 1)) {
            return redirect()->back()->with('error', 'Upaya pembelian terlampaui. Silakan coba lagi nanti.');
        }
        RateLimiter::hit("purchase:$userId", 60); // Allows only one withdrawal per 1 minute
        ...
        if ($package){
            ...
            if ($package->price <= $user->balance){
                ...
                $first_ref = User::where('id', Auth::user()->ref_by)->first();
                if ($first_ref){
                    ...
                    $second_ref = User::where('id', $first_ref->ref_by)->first();
                    if ($second_ref){
                        ...
                        $ledger->save();
                         ...
                         $third_ref = User::where('id', $second_ref->ref_by)->first();
                         if ($third_ref){
                             $amount = $package->price * setting('third_ref') / 100;
