...
<div style="width: 100%; margin: 0 auto; background: #1fc745c7; border-bottom: 0px solid #117546; " class="top">
...
<div style=" max-width:450px; margin:0 auto;margin-top: 70px;">
    ...
    <div style="">
        <ul>
            @foreach(\App\Models\Checkin::where('user_id', auth()->id())->where('amount','>',0)->orderByDesc('id')->get() as $element)
    ...
    border-radius: 7px;">
                    <div>
                        <h3 style="font-weight: bold">Checkin harian</h3>
                        <p>{{$element->created_at}}</p>
                    </div>
                    <div>
                        <h3 style="font-weight: bold">Rp {{number_format($element->amount)}}</h3>
                        <p class="sposp" style="text-transform: capitalize;">Checkin berhasil</p>
                    </div>
                </li>
            @endforeach
        </ul>
    ...
    @if(\App\Models\Checkin::where('user_id', auth()->id())->orderByDesc('id')->count() < 1 )
        <div style=" width:100%;margin:0 auto; position:relative;" id="itemlist">
            ...
            <div class="layui-flow-more">Tidak ada riwayat</div>
        </div>
    @endif
</div>
